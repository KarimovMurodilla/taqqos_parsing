...
def parse_maxcom_product():
    try:
        ...
        logging.error(f"Error in parse_maxcom_product: {e}")
...
def parse_pcmarket_category():
    try:
        ...
        logging.error(f"Error in parse_pcmarket_category: {e}")

@shared_task()
def parse_pcmarket_product():
    try:
        logging.info("----------Started pcmarket product parsing------")
        data = [category.url for category in get_all_categories(website="pcmarket")]
        pcmarket_product(data)
    except Exception as e:
        logging.error(f"Error in parse_pcmarket_product: {e}")
...
def parse_ikarvon_category():
    try:
        ...
        logging.error(f"Error in parse_ikarvon_category: {e}")
...
def parse_ikarvon_product():
    try:
        ...
        logging.error(f"Error in parse_ikarvon_product: {e}")
